...
public interface DebtRepository extends JpaRepository<LoanRequest, UUID> {
    ...
    @Query("SELECT COUNT(l) FROM LoanRequest l WHERE l.customer.id = :customerId AND l.status.name = 'DISBURSED'")
    Integer countActiveLoansByCustomerId(UUID customerId);
